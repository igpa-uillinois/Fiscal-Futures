...
rev_temp <- read_csv(file = paste0("FY", current_year, " Files/rev_temp.csv"))

exp_temp<- read_csv(file = paste0("FY", current_year, " Files/exp_temp.csv"))
```
...
exp_temp <- exp_temp |> mutate(drop_reason = ifelse(!is.na(refund), "Refund", drop_reason))
...
exp_temp <- exp_temp %>% 
  ...
  mutate(
    ...
    (obj_seq_type > "11590000" & obj_seq_type < "11660000") |  #~ "Object 1160-1165 Employer Contributions into Pension Fund; INCLUDED",     # objects 1159 to 1166 are all considered Retirement by Comptroller 
...
exp_temp <- exp_temp %>% 
  ...
  mutate(object = ifelse((pension ==TRUE & in_ff == "0"), "4431", object)) %>% 
  # changes weird teacher & judge retirement system  pensions object to normal pension object 4431
  mutate(pension =  ifelse(pension ==TRUE & in_ff == "0", TRUE, pension)) %>% # coded as 6 if it was supposed to be excluded. 
  mutate(in_ff = ifelse(pension==TRUE, "1", in_ff))
 ...
 # turns specific items into State Employee Healthcare 
  mutate(group_label = 
    ifelse(
     ...
     ( fund=="0907" & (agency=="478" & appr_org=="80") ), "State Employee Healthcare", agency_name),
    ...
    group = ifelse(group_label == "State Employee Healthcare", "904", as.character(agency)) )
...
exp_temp <- exp_temp %>% 
  mutate(
    ...
    group_label = 
      ...
      mutate(drop_reason = ifelse(group > "970" & group < "977", "Local Transfers", as.character(drop_reason)))
...
rev_temp <- rev_temp |> 
  mutate(drop_reason = ifelse(in_ff == "0", 
                               "Excluded Fund", NA))
...
write.csv(exp_temp, "budget_data_expenditures.csv")



rev_temp <- rev_temp |> select(-c(activefy2025, activefy2025_rev, type_assigned))
write.csv(rev_temp, "budget_data_revenue.csv")
```
