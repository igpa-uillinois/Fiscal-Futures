...
```{r}
exp_temp |> filter(agency == 370 & fy == 2025)
```
...
-   4470 for McCormick Place expansion project   
  -     from fund 0377   
...
```{r}
#| tbl-cap: "GO bond principal and GO bond interest"
...
```{r}
#| tbl-cap: "Short term borrowing principal and interest payments. "
#| 
...
Capital projects might be missing mccormick expansion project?
...
exp_temp %>%  
  filter(fy == 2025) |> 
  filter(fund != "0455" & (object == "8811" |object == "8813" | object == "8800")) %>%  # all principal, interest, and debt service except Tollway
...
```{r}
rev_temp %>% filter(rev_type == "72") %>% 
  ggplot() + 
  geom_col(aes(x=fy, y=receipts)) + labs(title = "All Bond Proceeds, Revenue Type = 72") +    
  scale_x_continuous(limits = c(1998, current_year), n.breaks = 6)
...
bond_proceeds %>% ggplot() + 
  ...
  scale_x_continuous(limits = c(1998, current_year), n.breaks = 6) +
  labs(title = "Bond Proceeds by Fund Type, Revenue Type = 72", y= "Billions of Dollars", color = element_blank() )
```
...
```{r}
rev_temp %>% 
  filter(rev_type == "72" & fy == 2025)

rev_temp %>% 
  filter(fy == 2025 & source_name == "BOND ISSUE PROCEEDS") |> summarize(receipts = sum(receipts))
...
```{r tollway2}
#| fig-cap: "Tollway Revenue and Expenditures. Bars in graph represent bond proceeeds revenue."
#
ggplot()+
  theme_classic() +
...
### Looking at Bond Proceeds

```{r}
rev_temp |> filter(rev_type == "72" & fy == 2025) |> 
  mutate(receipts = receipts / 1000000) |> 
  select(fund_name_ab, fund, source_name_AWM, receipts, fund_cat_name, agency_name) |> arrange(desc(receipts))

rev_temp |> filter(rev_type == "72" & fy == 2025) |> 
  summarize(receipts = sum(receipts) / 1000000) 

rev_temp |> filter(rev_type == "72" & fy == 2025) |> select(-c(data_source, fund_ab:activefy2025_rev, amnesty, local, type_assigned))
```


```{r}
#| tbl-cap: "Look at funds that received bond proceeds."


rev_temp |> filter(fy == 2025 & fund %in% c("0101", "0141","0270", "0551", "0553", "0554", "0825", "0959", "0971")) |>  mutate(receipts = round(receipts / 1000000) ) |>  arrange(desc(receipts))  |> select(-c(data_source, fund_ab:activefy2025_rev, amnesty, local, type_assigned))
```


```{r}
#| tbl-cap: "Looks at receipts in Build Illinois Bond Fund, Build Illinois Fund, and Build Illinois B R & I fund."
rev_temp |> filter(fy == 2025 & fund %in% c("0960", "0970", "0971")) |>  mutate(receipts = round(receipts / 1000000) ) |>  arrange(desc(receipts)) |> select(-c(data_source, fund_ab:activefy2025_rev, amnesty, local, type_assigned))
 


```

Bond proceeds ($2.7 billion) went to General Obligation BR&I, Build Illinois Bond, Transportation Bond Series A, Pension Acceleration Bond, Capital Development, Water Revolving, Transportation  Bond Series B, Multimodal Transportation Bond, and Anti-Pollution Funds. 


### Expenditure Transfers

```{r}
#| label: expendituretransfers
#| tbl-cap: "Expenditures that were transfers."

general_funds <- c("0001", "0005", "0412", "0007", "0640", "0644", "0686")

all_exp <- read_csv(paste0("data/FY", current_year, " Files/allexpfiles_2025.csv")) 


all_exp |> filter(agency == 799 & fy == 2025) |> 
  select(obj_seq_type, expenditure, fund_name, fund, agency, everything()) |> 
  mutate(expenditure = round(expenditure/1000000)) |> arrange(desc(expenditure)  ) 

all_exp |> filter(agency == 799 & fy == 2025) |> 
  group_by(obj_seq_type) |> 
  summarize(expenditure = round(sum(expenditure)/1000000)) |> arrange(desc(expenditure)  )
```
```{r}
all_exp |> filter(agency == 799 & fy == 2025) |> 
  summarize(trans_out = round(sum(expenditure/1000000)))

all_exp |> filter(agency == 799 & fy == 2025) |> 
  group_by(fund_name, fund)|>
  summarize(trans_out = round(expenditure/1000000)) |> arrange(desc(trans_out))
```


```{r}
#| label: revenuetransfers
#| tbl-cap: "Transfers into funds"

all_rev <- read_csv(paste0("data/FY", current_year, " Files/allrevfiles_2025.csv")) 

all_rev |> filter(agency == 799 & fy == 2025) |> select(fund_name, receipts,fund_name, fund, everything()) |> mutate(receipts = round(receipts/1000000)) |> arrange(desc(receipts)  )


```

```{r}
all_rev |> filter(agency == 799 & fy == 2025) |> select(fund_name, receipts,fund_name, fund, everything()) |> summarize(trans_in = round(sum(receipts/1000000)))
```

```{r}
#| label: tbl-revinto
#| tbl-cap: "Transfers into General Funds"

all_rev |> filter(agency == 799 & fy == 2025 & fund %in% general_funds) |> select(fund_name, receipts,fund_name, fund, everything()) |> mutate(receipts = round(receipts/1000000)) |> arrange(desc(receipts)  )
...
```{r}
#| tbl-cap: "Revenue into Fund 0101 (GO BR&I) and 0141 (Capital Development Fund). In FY2025, GO BR&I got $3.4 billion transfer into fund."

all_rev |> filter(fy == 2025 & fund %in% c("0141", "0101")) |> 
  select(source, fund_name, receipts,fund_name, fund, everything()) |> mutate(receipts = round(receipts/1000000)) |> arrange(desc(receipts)  )
```


### Move to different page - Transfers of Excess Funds 

Potential process change: Check transfers that have "EXCESS" in them and consider if they should be included or not? 
