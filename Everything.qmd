...
rev_temp <- rev_temp %>% f-ilter(!rev_type_new %in% drop_type)
...
write_csv(exp_temp, paste0("data/FY", current_year, " Files/exp_fy", current_year, "_pensionskeptascategory.csv"))
write_csv(rev_temp, paste0("data/FY", current_year, " Files/rev_fy", current_year, "_pensionskeptascategory.csv"))
...
ff_exp<- ff_exp %>% 
  select(any_of(-c(exp_971:exp_976)) ) # drop unwanted columns
...
ownsource_graph <- ggplot(ownsource_rev, aes(x=Year, y=Dollars/1000)) + 
  ...
  geom_line(data = fed_rev, aes(x=fy, y=fed_total/1000), color = "Black") + 
    ...
    scale_y_continuous(labels = comma)+
  scale_x_continuous(expand = c(0,0), limits = c(1998, current_year)) + 
  theme_classic()+
  theme(legend.position = "none",
    panel.background = element_rect(fill='transparent'), #transparent panel bg
    plot.background = element_rect(fill='transparent', color=NA), #transparent plot bg
...
write.xlsx(dataset_names, file = paste0("data/FY", current_year, " Files/summary_file_FY", current_year,"_w_pension_cat", todaysdate, ".xlsx"))
```
