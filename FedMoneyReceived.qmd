...
#| code-fold: false
cure_exp <- read_xlsx("./data/fedCUREexpenditures_updated.xlsx")


cure_exp2025 <- cure_exp %>%
  mutate(
         FY_Spent = factor(FY_Spent, levels = c("2020", "2021", "2022", "2023","2024", "2025")),
         FY_Received = factor(FY_Received, levels = c("2020", "2021", "2022", "2023","2024", "2025")))


cure_exp2025 %>% #expenditures per year
...
cure_exp2025  %>% 
  ...
  geom_text(stat = "stratum", aes(label = after_stat(stratum)), size = 2, reverse=FALSE) +
  labs(title = "Expenditures using State CURE funds: $11 billion spent FY20-FY25", 
  subtitle = "Year Received by State Department, Year Spent, and how it was spent so far", 
  caption = "Expenditures occured during FY21 through FY25. 
       Additional funds have been allocated and can be spent until FY26.
...
cure_exp2025  %>% 
 ...
 coord_flip()+
  ...
  labs(title = "State CURE Expenditures: Department that Received Revenue
       ...
       subtitle = "$11 billion spent by end of FY25")

cure_exp2025 %>% #expenditures for State government (with CURE $) and state departments
  group_by(State_local2)%>% 
  summarize(Expenditures=sum(`FY Expenditures`))

# State CURE & ESSER grants
cure_exp2025 %>% # total expenditures 
  summarize(Expenditures=sum(`FY Expenditures`)) 
```
...
cure_exp2025 %>% 
  group_by(FY_Spent, State_local, Fund )%>% 
  summarize(Expenditures=sum(`FY Expenditures`)) %>%
  pivot_wider(names_from = State_local, values_from = Expenditures) %>%
  arrange(FY_Spent)
```
...
exp_temp |> 
  ...
  group_by(fy, fund_name, trans_type) %>% 
  summarize(sum=round(sum(expenditure)),
...
exp_temp %>% filter(fy >2019 & (fund_name == "STATE CURE" | fund_name == "LOCAL CURE" | fund_name == "SBE FEDERAL DEPT OF EDUCATION" | fund_name == "DISASTER RESPONSE AND RECOVERY" | fund_name == "ESSENTIAL GOVT SERV SUPPORT" )) %>% 
  group_by(fy, fund_name, agency_name) %>% 
  summarize(sum=round(sum(expenditure)),
            appropriated = sum(appn_net_xfer)) %>% 
  arrange(-fy, -appropriated) # |> View()


exp_temp %>% filter(fy >2019 & (fund_name == "STATE CURE" | fund_name == "LOCAL CURE" | fund_name == "DISASTER RESPONSE AND RECOVERY" | fund_name == "ESSENTIAL GOVT SERV SUPPORT" )) %>% 
...
exp_temp %>% filter(fy == 2023 & (fund_name == "STATE CURE" | fund_name == "LOCAL CURE")) %>% group_by(org_name, agency_name, # object,
                                                                                                       wh_approp_name, fund_name) %>% summarize(sum=round(sum(expenditure))) %>% arrange(-sum)

exp_temp %>% filter(fy == 2022 & (fund_name == "STATE CURE"# | fund_name == "LOCAL CURE"
                                  )) %>% 
  group_by(agency_name, # object, 
                                                                                                       wh_approp_name, 
                                                                                                       fund_name) %>% summarize(sum=round(sum(expenditure))) %>% arrange(-sum)

exp_temp %>% filter(fy == 2023 & (fund_name == "STATE CURE" | fund_name == "LOCAL CURE")) %>% group_by(fund_name, #object, 
                                                                                                       org_name) %>% summarize(sum=round(sum(expenditure))) %>% arrange(-sum)


exp_temp %>% filter(fy == 2023 & (fund_name == "STATE CURE" | fund_name == "LOCAL CURE")) %>% group_by(fund_name, agency_name) %>% summarize(sum=round(sum(expenditure))) %>% arrange(-sum)

exp_temp %>% filter(fy == 2023 & (fund_name == "STATE CURE" | fund_name == "LOCAL CURE")) %>% group_by(fund_name, agency_name) %>% summarize(sum=round(sum(expenditure)))%>% arrange(-sum)

exp_temp %>% filter(fy == 2024 & (fund_name == "STATE CURE" | fund_name == "LOCAL CURE")) %>% group_by(fund_name, agency_name) %>% summarize(sum=round(sum(expenditure))) %>% arrange(-sum)

exp_temp %>% filter(#fy == 2025 & 
                      (fund_name == "STATE CURE" | fund_name == "LOCAL CURE")) %>% 
  group_by(fy, fund_name, agency_name) %>% summarize(sum=round(sum(expenditure))) %>% 
  arrange(-sum) |> filter(sum > 0) |> View()

exp_temp %>% filter(fy == 2021 & (fund_name == "STATE CURE" | fund_name == "LOCAL CURE")) %>% group_by(wh_approp_name, fund_name) %>% summarize(sum=round(sum(expenditure))) %>% arrange(-sum)
```

```{r}
# # without transfers
# exp_temp %>% filter(fy >2019 & (fund_name == "STATE CURE") & agency != 799) |> 
#   group_by(fy, 
#         #   agency_name
#            ) |> summarise(spent = round(sum(expenditure)))  |> filter(spent > 0)


exp_temp %>% filter(fy >2019 & (fund_name == "STATE CURE")) |> 
  group_by(fy#, fund_name, wh_approp_name ,
          # agency_name,
           ) |> summarise(spent = round(sum(expenditure)))  |> filter(spent > 0)
...
exp_temp %>% filter(fy >2019 & (fund_name == "STATE CURE")) |> 
  ...
  summarise(spent = round(sum(expenditure))) |> filter(spent > 0) |> View()
...
#  summarise(spent = round(sum(expenditure))) |> 
  #filter(spent > 0)
```
...
exp_temp %>% filter(fy > 2018 & fund_name == "SBE FEDERAL DEPT OF EDUCATION" & agency_name == "STATE BOARD OF EDUCATION") %>% group_by(wh_approp_name) %>% summarize(sum=round(sum(expenditure)))
...
K12 %>% summarize(esser_spent = round(sum(expenditure)))

K12 %>% group_by(fy) %>% summarize(esser_spent = round(sum(expenditure)))

K12 %>% group_by(fy, wh_approp_name) %>% summarize(esser_spent = round(sum(expenditure)))
```
