...
exp_variable_names <- read_xlsx("inputs/ioc_data_received/expenditures/Fis_Fut_Exp_2025_Final.xlsx") |> names()
exp_variable_names
...
exp_current <- read_xlsx("inputs/ioc_data_received/expenditures/Fis_Fut_Exp_2025_Final.xlsx") %>% 
  rename(fy = 'FISCAL YEAR',
         fund = 'FUND',
         fund_name = 'FUND NAME',
         agency = 'AGENCY',
         ...
         obj_seq_type = 'APPROPRIATION',
         wh_approp_name = 'APPROPRIATION NAME',
         appn_net_xfer = 'APPROPRIATED AMT NET OF TRANSFERS',
         expenditure = 'EXPENDED AMT') %>%
  mutate( data_source = "exp IOC 12.02.2025"
  )  |> 
...
```{r historic-create-rev-csv-dtafiles, include=FALSE, eval=FALSE}
...
13 revenue funds were in new revenue data that were not in past data.
Remember, some of them maybe be false alarms and have small fund name changes.
...
Remember: `allrevfiles` and `allexpfiles` contain the data received from the comptroller without recoding agencies or funds that have changed between fiscal years. Variables were renamed when needed to have consistent names but funds and agencies have NOT been recoded for consistentcategories using Fiscal Future categories in these files.
:::
...
# Variables below only used for inspecting transfers. Transfers are excluded when 
# filtering out agency 799 for Fiscal Gap calculations, 
# but check for trans_type =="EXCESS" for extra funds that moved back into General Revenue Fund
# (Income tax refund excess amounts were flagged by GOMB.)
...
-   the initial combined years of data are saved as dataframes named `exp_allyears` and `rev_allyears`. These are then saved as exp_temp and rev_temp while recoding variables. This is BEFORE creating Fiscal Future category groups and further data cleaning done in the Calculating the Fiscal Gap page.
